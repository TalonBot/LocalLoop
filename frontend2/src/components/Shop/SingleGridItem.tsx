...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  const isInWishlist = wishlistItems.some((wish) => wish.id === item.id);
  ...
  const handleAddToCart = () => {
    dispatch(
      addItemToCart({
        ...
        quantity: 1,
        // status: item.is_available ? "available" : "unavailable", // <-- REMOVE THIS LINE
      })
    );
  };
  ...
  const handleItemToWishList = () => {
    dispatch(
      addItemToWishlist({
        // Map Product fields to WishListItem fields
        id: item.id,
        title: item.name,
        price: item.price,
        discountedPrice: item.price, // or 0 if you want
  ...
  return (
    <div className="group h-[400px] flex flex-col bg-white rounded-lg shadow-1 p-4">
      <div>
        <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-gray-100 min-h-[260px] h-[260px] mb-4">
          {item.product_images?.[0]?.image_url ? (
            <Image
              ...
              width={240}
              height={240}
              className="object-cover w-[240px] h-[240px] rounded"
              quality={70}
              priority={false}
            />
          ) : (
            <div className="w-[240px] h-[240px] bg-gray-200 flex items-center justify-center text-gray-400">
              No Image
            </div>
          )}
        ...
        <h3 className="font-medium text-dark ease-out duration-200 hover:text-blue mb-1.5 line-clamp-2 h-[48px]">
